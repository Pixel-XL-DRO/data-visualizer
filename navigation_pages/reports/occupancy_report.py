...
(attraction_groups, selected_streets, start_date, end_date, granularity) = occupancy_report_sidebar.filter_data(df_initial, df_locations)
...
with st.spinner("Ładowanie danych rezerwacji...", show_time=True):
  df_reservations, df_slots_occupancy = utils.run_in_parallel(
    (occupancy_report_queries.get_reservations_data, (selected_streets, attraction_groups, start_datetime, end_datetime)),
...
if granularity == "Godzina":
  ...
  df_all['start_date_hour'] = df_all['hour_key'].apply(utils.parse_hour)
  ...
  if granularity == "Dzień":
    ...
    iso = df_all['_dt'].dt.isocalendar()
    df_all['_year'] = iso.year
    df_all['_week'] = iso.week
    df_all['sort_key'] = pd.to_datetime(df_all['_dt'].dt.to_period('W').dt.start_time)
    df_all['display_label'] = df_all.apply(lambda r: f"W{int(r['_week'])} {int(r['_year'])}", axis=1)
