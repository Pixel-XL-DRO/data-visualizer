...
def render_online_view(df):
    ...
    df_online, _, _, _ = income_utils.group_data_cumulative(df, df_dotypos, dfv, moving_average_days, group_by, start_date, end_date)
    df_by_weekday = income_utils.average_by_weekday(df, df_dotypos, dfv, group_by, group_dates_by, start_date, end_date)
...
def render_pos_view(df_dotypos):
    ...
    _, df_pos, _, _ = income_utils.group_data_cumulative(df, df_dotypos, dfv, moving_average_days, groupBy, start_date, end_date)
    df_by_weekday = income_utils.average_by_weekday(df, df_dotypos, dfv, groupBy, group_dates_by, start_date, end_date)
...
def render_total_view(df, df_dotypos):
    ...
    _, _, df_total, _ = income_utils.group_data_cumulative(df, df_dotypos, dfv, moving_average_days, groupBy, start_date, end_date)
    df_by_weekday = income_utils.average_by_weekday(df, df_dotypos, dfv, groupBy, group_dates_by, start_date, end_date)
...
def render_voucher_view(dfv):

    (dfv, separate_cities, show_only_moving_average, moving_average_days, moving_average_toggle, group_dates_by, start_date, end_date) = income_sidebar.filter_voucher_data(dfv)
    groupBy = 'city' if separate_cities else None

    _, _, _, df_voucher = income_utils.group_data_cumulative(df, df_dotypos, dfv, moving_average_days, groupBy, start_date, end_date)
    df_by_weekday = income_utils.average_by_weekday(df, df_dotypos, dfv, groupBy, group_dates_by, start_date, end_date)

    voucher_chart = utils.create_chart_new(df_voucher, df_voucher['date'], "Data", 'total_voucher_sum' if not show_only_moving_average else None, 'total_voucher_sum_ma' if moving_average_toggle else None, "Przychód (PLN)", groupBy, 2 if groupBy else 4, "Średnia", False)
    ...
    reservations_chart = utils.create_chart_new(df_voucher, df_voucher['date'], "Data", None, 'cumsum_voucher', "Kumulujacy sie przychód (PLN)", groupBy, 2 if groupBy else 4, "Przychód (PLN)", False)
    st.plotly_chart(reservations_chart, use_container_width=True)
