...
SAFI_CITIES = ["lubicz", "ogrodowa", "kijowska", "swietego-marcina", "sokolska", "grunwaldzka"]
...
def render_safi_view(
  ...
  with st.spinner("Ładowanie danych...", show_time=True):
    ...
    df, df_slots_occupancy = utils.run_in_parallel(
      (boards_occupancy_queries.get_reservations_data, (attraction_groups, selected_week_start, selected_week_end)),
      (queries.get_slots_occupancy, (selected_week_start, selected_week_end))
    )

  occupancy_locations_map = {}


  # print(df)
  
  df = df[df['street'].isin(SAFI_CITIES)]

  for location in df["street"].unique():
    selected_location = df_locations[df_locations['street'] == location]
    ...
    dataframe = df[df['street'] == location]
    ...
    for _, reservation in dataframe.iterrows():
    ...
    for index, row in daily_avg.iterrows():
      ...
      average_dataframe = pd.concat([average_dataframe, df_label], ignore_index=True)

    heatmap_df = pd.concat([heatmap_df, average_dataframe], ignore_index=True)
  
    occupancy_locations_map[location] = heatmap_df

  heatmap = alt.Chart(occupancy_locations_map[city_selection]).mark_rect(stroke="black", strokeWidth=3).encode(
  ...
  display_df = pd.concat(
    [
        df.loc[df["sort_key"].isna(), ["boards_occupancy", "start_date_key"]]
          .assign(street=street)
        for street, df in occupancy_locations_map.items()
    ],
    ignore_index=True
  )[["street", "boards_occupancy", "start_date_key"]]

  display_df["street"] = display_df["street"].replace(utils.street_to_location)  
  ...
  chart = (
    ...
    .encode(
        ...
        yOffset=alt.YOffset("start_date_key:N"),
        x=alt.X(
          "boards_occupancy:Q",
          title="Zajętość [%]"
        ),
        ...
        tooltip=[
            alt.Tooltip("street:N", title="Lokacja"),
            alt.Tooltip("start_date_key:N", title="Data"),
  ...
  st.write("Zajętość dnia tygodnia wg miasta")  
