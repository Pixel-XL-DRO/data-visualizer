...
- A human friendly feature title in snake_case (e.g. new_feature)
- A safe git branch name not already taken (e.g. claude/feature/snake_case)
...
2. `feature_slug`
   ...
   - Example: `income-report` or `reservations-chart`.

3. `branch_name`
   - Format: `claude/feature/<feature_slug>`
   - Example: `claude/feature/income-report`.
...
Before making any content, switch to a new Git branch using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `claude/feature/income-report-01`
