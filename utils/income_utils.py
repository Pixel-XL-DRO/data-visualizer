...
def group_data_cumulative(df, df_dotypos, moving_average_days, grouping_field, start_date, end_date):
    ...
    if grouping_field:
        ...
        if grouping_field != 'city':
            ...
            df_total['total_reservations_sum'] = df_total['total_online_sum'] + df_total['total_pos_sum']
    ...
    return df_online, df_pos, df_total

def average_by_weekday(df, df_dotypos, grouping_field, grouping_type, start_date, end_date):
    ...
    total_periods = max(online_periods, pos_periods)

    online_daily = df.groupby('group').agg(
        online_sum=('whole_cost_with_voucher', 'sum'),
    ).reset_index()

    pos_daily = df_dotypos.groupby('group').agg(
        pos_sum=('brutto', 'sum'),
    ).reset_index()
    ...
    daily['total_reservations_mean'] = (daily['online_sum'] + daily['pos_sum']) / total_periods
