...
def group_order_items(df, group_by, moving_average_days):
    ...
    if group_by:
        ...
        df_grouped['netto_rolling_avg'] = df_grouped.groupby(group_by)['netto'].transform(
            lambda x: x.rolling(window=moving_average_days, min_periods=1).mean())
        ...
        df_upsell_grouped['avg_netto_per_purchase'] = df_upsell_grouped.groupby(group_by).apply(
            lambda g: (g['netto'].rolling(window=moving_average_days, min_periods=1).sum() /
                       g['quantity'].rolling(window=moving_average_days, min_periods=1).sum())
        ).reset_index(level=0, drop=True)

        df_upsell_grouped['avg_brutto_per_purchase'] = df_upsell_grouped.groupby(group_by).apply(
            lambda g: (g['brutto'].rolling(window=moving_average_days, min_periods=1).sum() /
                       g['quantity'].rolling(window=moving_average_days, min_periods=1).sum())
        ).reset_index(level=0, drop=True)
        ...
        df_grouped['netto_rolling_avg'] = df_grouped['netto'].rolling(
            window=moving_average_days, min_periods=1).mean()
        ...
        df_upsell_grouped['avg_netto_per_purchase'] = (
            df_upsell_grouped['netto'].rolling(window=moving_average_days, min_periods=1).sum() /
            df_upsell_grouped['quantity'].rolling(window=moving_average_days, min_periods=1).sum()
        )

        df_upsell_grouped['avg_brutto_per_purchase'] = (
            df_upsell_grouped['brutto'].rolling(window=moving_average_days, min_periods=1).sum() /
            df_upsell_grouped['quantity'].rolling(window=moving_average_days, min_periods=1).sum()
        )
